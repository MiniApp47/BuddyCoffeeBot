...
`;
    ...
    function renderConfirmation() {
        ...
        if (promoInputContainer && promoAppliedContainer) {
            if (appliedPromo) {
                promoInputContainer.style.display = 'none';
                promoAppliedContainer.style.display = 'flex';
                const promoAppliedText = document.getElementById('promo-applied-text');
                if (promoAppliedText) promoAppliedText.innerText = `Code "${appliedPromo}" appliqué !`;
            } else {
 ...
 function populateFilters() {
    ...
    document.body.addEventListener('click', function (e) {
        ...
        const copyContactButton = target.closest('.contact-copy-btn');
        if (copyContactButton) {
            const textToCopy = copyContactButton.dataset.copy;
            const fallbackCopy = () => {
                ...
                textarea.remove();
                showNotification(`✅ ${textToCopy} copié !`);
            };

            if (navigator.clipboard && navigator.clipboard.writeText) {
                navigator.clipboard.writeText(textToCopy)
                    .then(() => showNotification(`✅ ${textToCopy} copié !`))
                    .catch(fallbackCopy);
            } else {
                fallbackCopy();
            }
